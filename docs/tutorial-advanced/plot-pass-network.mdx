...
```jsx
const HomeAwaySelector = ({ homeAway, setActiveTeamId, activeTeamId }) => {
  return (
    <div>
      <label>
        <input
...
export const PassNetwork = () => {
  ...
  const onMatchChange = async (match) => {
    ...
    const filteredData = passNetwork.filter(
      (pass) => pass.teamId == match.home_team.home_team_id
    );
  ...
  const onActiveTeamChange = (teamId) => {
    ...
    pitch.removeAllLayers();
    const filteredData = passNetwork.filter((pass) => pass.teamId == teamId);
    Rabona.layer({
      type: "ballMovement",
      data: filteredData,
      options: {
  ...
  return (
    <>
      ...
      <HomeAwaySelector
        homeAway={homeAway}
        setActiveTeamId={onActiveTeamChange}
